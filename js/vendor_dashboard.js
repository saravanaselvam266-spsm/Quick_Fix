...
if (!user) {
  showToast(
    "Login Required",
    "Please login to access the mechanic dashboard",
    "info",
  );
...
async function loadMechanicDashboard() {
  ...
  try {
    ...
    const bookingRes = await fetch(
      `${API_BASE_URL}/bookings/vendor/${vendorId}`,
      requestOptions,
    );
    ...
    allBookings.forEach((b) => {
      ...
      if (b.status === "completed") {
        ...
        if (
          bookingDateObj.getDate() === todayObj.getDate() &&
    ...
    if (document.getElementById("todayEarnings"))
      document.getElementById("todayEarnings").innerText = "₹" + todayEarnings;
    if (document.getElementById("weekEarnings"))
      document.getElementById("weekEarnings").innerText = "₹" + weekEarnings;
    if (document.getElementById("activeJobs"))
      document.getElementById("activeJobs").innerText = activeJobsCount;
    ...
    const response = await fetch(
      `${API_BASE_URL}/bookings/nearby?lat=${myLat}&lon=${myLon}&radius=${radius}`,
      requestOptions,
    );
    ...
    if (myActiveJobs.length === 0) {
      jobList.innerHTML +=
        "<p style='color:#777; font-size:14px; margin-bottom:20px;'>No active jobs currently.</p>";
    } else {
    ...
    if (nearbyBookings.length === 0) {
      jobList.innerHTML +=
        "<p style='color:#777; font-size:14px;'>No new requests nearby.</p>";
    } else {
...
function createJobCard(booking, serviceMap, userMap, isActive) {
  ...
  const serviceName =
    serviceMap[booking.service_id] || `Service #${booking.service_id}`;
  const customerName =
    userMap[booking.customer_id] || `Customer #${booking.customer_id}`;
...
async function acceptBooking(bookingId, btnElement) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/bookings/${bookingId}/accept?vendor_id=${vendorId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${user.access_token}`,
    ...
    if (response.ok) {
      showToast(
        "Job Accepted!",
        "You have successfully accepted this job. Check 'Active Jobs'.",
        "success",
      );
      ...
      showToast("Error", err.detail || "Could not accept booking", "error");
    }
  } catch (error) {
    if (btnElement) toggleLoading(btnElement, false);
