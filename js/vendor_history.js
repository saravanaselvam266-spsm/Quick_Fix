...
if (!user) {
  showToast("Login Required", "Please login to view your job history", "info");
...
async function loadVendorHistory() {
  ...
  try {
    ...
    completedJobs.forEach((job) => {
      ...
      const serviceName =
        serviceMap[job.service_id] || `Service #${job.service_id}`;
      const customerName =
        userMap[job.customer_id] || `Customer #${job.customer_id}`;
